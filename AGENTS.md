...
- Styling. Colors per the token rule above; token definitions and composition rules in `context/design.md`. Radius scale: small controls `rounded-md`, panels `rounded-lg`/`rounded-xl` per registry. Lucide icons; icon-only buttons need an accessible label.
