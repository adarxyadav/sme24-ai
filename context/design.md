...
A completed run's page is one report (T-039, mockup `context/product/design-report-page.html`), not a stack of cards. Between the masthead meta line and the strip sits **the lead** (T-040): 2–4 sentences of deterministic prose assembled in code from the stored facts — disclosure count, benchmark rank on the rate metric, the ≈-prefixed cost range, matched experts and proposal tier. Every figure is the strip's own value restated; a fact that does not exist drops its sentence entirely (never hedged), and the thinnest run still opens with the disclosure sentence. No model call ever writes it. Signature: **the findings line** — a five-entry masthead strip under the company h1, the whole report as one line of mono figures in pipeline order, each entry an anchor to its section; an entry with nothing to say renders muted instead of dressing an absence as a number. Sections chain under numbered chapter rules (mono number + title + hairline) and each keeps its instrument: the seven-tick disclosure meter (ledger), the position-only rank strip (benchmark — the company the only filled mark, peers as outline dots, no competing hues), the cost rail (invoice pass — severity segments of the max total, the lost-time spread hatched), teal fit bars with the score shown (experts), and the report's single filled-teal action closing the page (proposal PDF). Cards are `bg-card` `rounded-lg` hairline panels with `px-5` edge padding; eyebrow headers are 11px uppercase `muted-foreground`; figures are `font-mono tabular-nums`; confidence and availability are toned dots (outline dot for absence), row hovers tonal `accent/50`. Below `md` the strip stacks to label-left/value-right rows and the ledger stacks each metric into the two-line grammar; other tables keep horizontal scroll inside the card. Empty states stay in the same card grammar with a muted hero sentence — never a dressed-up number.
