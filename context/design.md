...
`/dashboard`'s intake is a single ask bar anchored in the upper third of the inset (T-037, mockup `context/product/design-search-composer.html`) — anchored, not centered, so the floating panel below always has room and the block never re-centers: greeting + one large autofocused company-name input in a hairline `rounded-xl` panel. Everything optional lives behind three quiet footer controls — attach (paperclip; the PDF renders as a removable chip), and the `Website` / `Your figures` disclosures, which float below the bar as panels (`popover` tokens) — the composer never grows and the page never reflows. One panel open at a time; the figures panel is one compact grid (3-up ≥ `sm`, 2-up below) with a single shared basis line instead of per-field hints, sized to fit the desktop viewport with no internal scrolling (per-metric bases still render in the ledger); the fit fallback stays — max-height measured to the fold on open/resize — so on genuinely short screens (phones) it scrolls inside itself, never past the screen; Escape closes and returns focus to the chip, so does any press outside the composer. Closed sections stay mounted so typed values always submit; a closed section holding a value marks its chip with a teal dot. Teal appears exactly once: the submit circle (spinner while pending). Chip open state is tonal (`accent`), never teal. Errors are one line under the composer.
