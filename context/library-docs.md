...
- Captcha: Supabase Auth's captcha protection (Turnstile) is on; `signInWithOtp` must carry `options.captchaToken` or Supabase answers `captcha_failed`. The widget is `@marsidev/react-turnstile` in `LoginCard`, the token a hidden form field; the secret is never in the repo (`t-024-spec.md`).
...
- **Uploads.** Client documents go through `POST /api/uploads` (session verified, `%PDF-` magic bytes checked, service-role upload to `<user_id>/<uuid>.pdf`), never a browser Storage client; the bucket has no client policy at all and is read only by the pipeline (`t-020-spec.md` D1).
...
- PDF input: pass the bytes as a `file` content part (`mediaType: "application/pdf"`) in `messages`, with `Output.object` as usual; works through the Gateway on the current model (`t-020-spec.md`).
...
- One generic create/result pair, two entry points: `researchCompany` (EHS schema, stage 1) and `benchmarkPeers` (peer schema, stage 3, processor `base`). A new Parallel call adds a schema file + an entry point, not a second loop.
- `researchCompany` and `benchmarkPeers` take `onRunCreated`, fired after the create call and before the result loop — the task logs the Parallel run id there, so a run that dies mid-wait still names the paid run.
