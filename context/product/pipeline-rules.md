...
- `queued` has an owner: a scheduled sweeper (`trigger/sweep-queued-runs.ts`) re-triggers runs still `queued` past a staleness threshold, because `onFailure`/`onCancel` both require the task to have started and so cover nothing that never reached a worker. A task claims its run with a conditional `queued -> researching` update, which makes the status column — not Trigger.dev's idempotency store — the proof that work already started, so a redundant re-trigger is a no-op. The claim is attempted for every trigger that means "start this work" (route enqueue, sweeper re-trigger — the payload's `reason` marks them), never gated on the status the task happened to read, since a re-trigger usually starts long after that read; an escalation re-run is exempt, being a deliberate re-entry at the current status. A retry of the claiming run is not a duplicate: the claim also accepts `researching` when the row's `trigger_run_id` is the caller's own run id, so a retry after a post-claim throw re-enters the work instead of exiting. A run whose enqueue has failed three times is set `failed` through the same terminal shape as above — failures are counted, not sweeps, because a run waiting behind its owner's concurrency-1 queue is re-triggered successfully every sweep and is healthy, not stuck.
- A child stage that leaves the run in a working status for the next stage hands the handle back first: its last act writes `trigger_run_id` = the waiting parent's run id, guarded on its status and its own handle. A finished child's id must never be what a working row names, or the stalled sweeper (next rule) reads a dead task and fails a healthy run (t-018-spec.md D4).
- Every working status after `queued` has an owner too: a scheduled sweeper (`trigger/sweep-stalled-runs.ts`) asks Trigger.dev (`runs.retrieve`) whether the run named by the row's `trigger_run_id` — written inside the claim that moved the status, and overwritten by stage 2's claim so the child decides at `extracting` — is still alive, and sets `failed` through the same terminal shape when it is not (crashed, cancelled with no worker for `onCancel`, returned without a terminal). No age threshold: a run may legitimately hold a working status for the task's full `maxDuration`. A working row with no handle is dead by construction. The write is conditional on the working status and the handle it asked about; uncertainty never fails a run.
...
- **Out:** `benchmarks` record `{ peer_count, rank, verdict, maturity_label, maturity_rationale, per_metric_comparison }` — `rate_metric`, `references`, and peer metadata live inside `per_metric_comparison`. The run moves `extracting -> benchmarking` on entry and stays there for stage 4. Only references on the per-1'000'000-hours base reach the judgment call, so a rationale never cites a figure the report cannot show.
