...
- `/dashboard/runs/[id]` (app/dashboard/runs/[id]/) — `RunStatusCard` for a run the caller owns, `RunProgress` while it is queued or in progress, plus `KpiLedger`, `IncidentCostCard`, `BenchmarkCard` and `ExpertMatchesCard` (the last two when a benchmark row exists) when the run is completed; another user's run is not-found. noindex.
