...
- `SiteHeader` (components/marketing/SiteHeader.tsx) — top bar with brand link (`BrandMark`, `aria-label` on the link) and a primary `<nav>` holding a Packages anchor link and `AuthNav`; no props.
- `SiteFooter` (components/marketing/SiteFooter.tsx) — copyright + tagline, a footer nav (How it works, Packages, For experts, Sign in) and the `ThemeToggle`; no props.
